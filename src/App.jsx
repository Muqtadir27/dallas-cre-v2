...
function computeStats(properties) {
  ...
  const avgValue = Math.round(properties.reduce((s, p) => s + (p.predicted_value || p.list_price || 0), 0) / total);
  ...
  const totalValue = properties.reduce((s, p) => s + (p.predicted_value || p.list_price || 0), 0);
  ...
  properties.forEach(p => {
    ...
    nbrMap[p.neighborhood].totalValue += (p.predicted_value || p.list_price || 0);
  ...
  properties.forEach(p => {
    ...
    typeMap[p.property_type].totalValue += (p.predicted_value || p.list_price || 0);
  });
...
export default function App() {
  ...
  const [mapMode, setMapMode] = useState('clusters'); // clusters default for performance
  ...
  const properties = useMemo(() => {
    ...
    return allProperties.filter(p => {
      ...
      if (filters.minValue && (p.predicted_value || p.list_price || 0) < Number(filters.minValue)) return false;
      if (filters.maxValue && (p.predicted_value || p.list_price || 0) > Number(filters.maxValue)) return false;
      return true;
    });
      ...
      <div className={styles.body}>
        ...
        <main className={styles.main}>
          ...
          {activeView === 'recommendations' && (
            <RecommendationsPanel onSelectProperty={(p) => {
